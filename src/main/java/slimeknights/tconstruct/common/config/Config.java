...
public final class Config {
  ...
  public static boolean syncConfig() {
    ...
    // Worldgen
    {
      ...
      int[] slimeIslandBlacklist = slimeIslandDimensions;

      prop = configFile.get(cat, "slimeIslandBlacklist", slimeIslandBlacklist);
      prop.setComment("DEPRECATED: Prevents generation of slime islands in the listed dimensions (overrides `slimeIslandDimensions` and `slimeIslandDimensionsIsBlacklist` if modified)");
      slimeIslandBlacklist = prop.getIntList();
      propOrder.add(prop.getName());

      boolean deprecatedSlimeIslandBlacklistBehaviour = !prop.isDefault();
      ...
      if(deprecatedSlimeIslandBlacklistBehaviour) {
        slimeIslandDimensions = slimeIslandBlacklist;
        slimeIslandDimensionsIsBlacklist = true;
      }
